...
export function registerDocument(app) {
  ...
  register('API - set data in empty document', async () => {
    const _ = db.command;
  ...
  register('API - set data in document existed', async () => {
  ...
  register('API - remove document that not exist', async () => {
    const document = db.collection(collName).doc(docIDGenerated);
    const data = await document.remove();
    assert(!data.deleted);
  });

  register('API - remove document should success', async () => {
